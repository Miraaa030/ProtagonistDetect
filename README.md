...
If anyone wants to add the fusion module: modify `multimodal_fusion.py`.

The output dictionary of `ProtagonistDetector.forward()` keeps the following keys unchanged:
- `rgb`: shape `[B, T, 1024]`
- `embeds`: shape `[B, 10, 128]`
- `audio`: shape `[B, T, 128]`
- `final_score`: shape `[B, 10]`
- `mask`: shape `[B, T]`
- `seg_mask`: shape `[B, 10]`
- `length`: shape `[B]`
